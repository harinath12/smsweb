...
{
	/*
	SELECT *  FROM `teacher_academic` WHERE `position` LIKE 'Driver' ORDER BY `id`  DESC
	*/
...
LEFT JOIN `teacher_academic` ON users.id = teacher_academic.user_id
WHERE `email`='$email' AND `password`='$password_md5' AND `delete_status`='1' AND `position` LIKE 'Driver'";
