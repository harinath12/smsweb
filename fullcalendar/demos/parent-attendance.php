...
{
	$calendar_title = $calendar_fetch['calendar_title'];
...
$att_sql="SELECT * FROM `attendance` WHERE `user_id` = '$user_id' ";
$att_exe = mysql_query($att_sql);
$att_cnt = @mysql_num_rows($att_exe);
while($att_fetch = mysql_fetch_array($att_exe))
{
    ...
    $calendar_list .= "start: '$calendar_start_date',";
	$calendar_list .= "end: '$calendar_end_date',";
	$calendar_list .= "className: ['event', 'presentEvent']";
	$calendar_list .= "},";
	}
	if($calendar_type==2) {
	$calendar_list .= "{";
	...
	$calendar_list .= "className: ['event', 'leaveEvent']";
	} else {
	$calendar_list .= "className: ['event', 'absentEvent']";
	}
