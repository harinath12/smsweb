...
<script>
    $(function() {
        $('#subjectId').change(function() {
			...
			if(term!="") {
            $('#chapter').empty();
			console.log({cid: clsId, sub: sub, term: term});
            $.get('chapterscriptlist.php', {cid: clsId, sub: sub, term: term}, function(result){
                var sublist = "<option value=''>Select Chapter</option>";
                $.each(JSON.parse(result), function(i,item) {
                    sublist = sublist + "<option value='" + item.cname + "'>" + item.cname + "</option>";
                });
