...
// This is the file path to be uploaded.
    ...
    for ($i = 2; $i <= $arrayCount; $i++) {
        $admissionNum = trim($allDataInSheet[$i]["C"]);
        $photo = trim($allDataInSheet[$i]["B"]);
        $filepath = 'upload/studentphoto/'.$photo . '.JPG';
        ...
        if (!empty($photo)) {
            $update_sql = "UPDATE student_general set photo='$filepath' where admission_number='$admissionNum'";
