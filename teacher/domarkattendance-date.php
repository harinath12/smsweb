...
for ($i = 0; $i < $cnt; $i++) {
    ...
    if($stud_cnt == 0){
        echo $attendance_sql = "INSERT INTO `attendance` (user_id, class_id, section_name, forenoon, afternoon, attendance_date, fn_entry_status, an_entry_status, remarks, leave_status, created_by, created_at) VALUES
('$studId','$className', '$sectionName', '$forenoon', '$afternoon', '$date', '$fn_entry_status', '$an_entry_status','$remarks', '$leave_status', '$username', '$cdate')";
        $attendance_exe = mysql_query($attendance_sql);
    }
    else{
        $update_remarks_sql = "update attendance set forenoon = '$forenoon', afternoon='$afternoon', remarks = '$remarks', updated_by='$username', updated_at = '$cdate' where user_id='$studId';";
    ...
    if($forenoon='off' || $afternoon='off'){
        if(!empty($remarks)){
            ...
            if($leave_cnt>0){
                ...
                $insert_leave_sql = "insert into student_leave (student_id,title, leave_from_date, created_by, created_at) values ('$studId','$remarks','$date','$username','$cdate');";
...
header("Location: attendance-date.php?succ=1");
