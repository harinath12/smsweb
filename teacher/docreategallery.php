...
for ($i = 0; $i < $cnt; $i++) {
    if (isset($_FILES['gallery'.$i])) {
        ...
        if (!empty($base)) {
            ...
            $mvetarget = '../admin/upload/gallery/' . $newname;
            $target = 'upload/gallery/' . $newname;
            $moveFile = move_uploaded_file($_FILES['gallery'.$i]['tmp_name'], $mvetarget);
        }
...
('$className', '$galleryTitle', '$galleryDesc', '$target', '$date','1',0,'$username', '$date')";
