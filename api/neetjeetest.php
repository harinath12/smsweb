<?php
function get_aptitude_questions(){
	...
	if($student_id && $test_id){
		...
		$ques_sql=mysql_query("SELECT * FROM `aptitude_question_answer` where id IN (SELECT question_id FROM `aptitude_test_question` WHERE `daily_test_id`='$test_id') and question_type='Choose'");
...
function get_aptitude_review(){
	...
	if($student_id && $test_id && $answer_id){
		...
		$ques_sql=mysql_query("SELECT * FROM `aptitude_question_answer` where id IN (SELECT question_id FROM `aptitude_test_question` WHERE `daily_test_id`='$test_id')");
        $ques_cnt = mysql_num_rows($ques_sql);
        if($ques_cnt> 0)
		{
			while($choose_fet = mysql_fetch_assoc($ques_sql)){
