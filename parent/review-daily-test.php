...
$ques_sql = "select * from daily_test_answer where student_id='$user_id' and daily_test_id='$test_id' order by id desc";
                                ...
                                if($ques_cnt>0)
                                {
                                    ...
                                    while($ques_fet=mysql_fetch_array($ques_exe))
                                    {
                                        ...
                                        <tr>
											...
											<?php 
											$mark_id=$ques_fet['id'];
											$mark_query="SELECT SUM(daily_test_mark) AS mark FROM `daily_test_question_answer` WHERE `daily_test_answer_id`='$mark_id'";
											$mark_query_exe=mysql_query($mark_query);
											$mark_query_fet=mysql_fetch_assoc($mark_query_exe);
											
											echo $mark_query_fet['mark']; 
											?>
											
											</td>
											<td class="text-center">
                                                <ul class="icons-list">
                                                    <li><a href="review-daily-test-view.php?test_id=<?php echo $test_id; ?>&answer_id=<?php echo $ques_fet['id']; ?>" title="View"><button type="button" class="btn btn-info btn-xs"><i class="fa fa-eye"></i> View </button></a></li>
                                                </ul>
                                            </td>
                                        </tr>
                                    <?php
